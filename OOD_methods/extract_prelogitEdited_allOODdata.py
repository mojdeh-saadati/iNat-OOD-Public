...
def extract_prelogit(args):
    ...
    with torch.no_grad():
        ...
        outDistValidStr1 = args.outdist_test_path1
        outDistValidStr2 = args.outdist_test_path2
        ...
        """
        outDistValid1 = datasets.ImageFolder(outDistValidStr1,preprocessing)
        outDistValid_loader1 = DataLoader(outDistValid1, batch_size = 256, shuffle = False, num_workers=8, pin_memory=True)

        """
        outDistValid2 = datasets.ImageFolder(outDistValidStr2,preprocessing)
        outDistValid_loader2 = DataLoader(outDistValid2, batch_size = 256, shuffle = False, num_workers=8, pin_memory=True)

        outDistValid3 = datasets.ImageFolder(outDistValidStr3,preprocessing)
        outDistValid_loader3 = DataLoader(outDistValid3, batch_size = 256, shuffle = False, num_workers=8, pin_memory=True)
        ...
        """
        step1 = time.time()        
        outDistValid_embeds1, _ , _ = standalone_get_prelogits(modelN, outDistValid_loader1)
        step2 = time.time()
        print("finish third Duration", step2 - step1)
        torch.save(torch.tensor(outDistValid_embeds1),args.logits_path+args.checkpoints+"OODInsect.pt")

        step1 = time.time()        
        #outDistValid_embeds2, _ , _ = standalone_get_prelogits(modelN, outDistValid_loader2)
        step2 = time.time()
        print("finish third Duration", step2 - step1)
        #torch.save(torch.tensor(outDistValid_embeds2),args.logits_path+args.checkpoints+"imagenet_2012.pt")
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--outdist-test-path1", default="/datasets01/imagenet_full_size/061417/", type=str, help="")
    parser.add_argument("--outdist-test-path2", default="/datasets01/imagenet_full_size/061417/", type=str, help="")    
