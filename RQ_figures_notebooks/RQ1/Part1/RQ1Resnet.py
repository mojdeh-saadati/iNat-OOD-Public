...
chpsResnet = ["88.87","91.45","95.12","97.54"] *3
...
for path,alg, chp, safe, risky in zip(paths,algs,chpsResnet, safeL, riskyL):
    ...
    result = result.append({'checkpoints' : chp, 'OODalg' : alg, "AUROC" : a[0], "AUPR": a[1],"FPR95":a[2]}, ignore_index = True)     
print("result === " , result) 
result.to_csv('/work/baskarg/Mojdeh/iNat_Project-mini-Insecta-2021/Models/Resnet50-142/resnet50_result.csv') 



plt.plot(result.checkpoints[result['OODalg'] == "MSP"], result.AUROC[result['OODalg'] == "MSP"] , label = "MSP");
plt.plot(result.checkpoints[result['OODalg'] == "EBM"], result.AUROC[result['OODalg'] == "EBM"] , label = "EBM");
plt.plot(result.checkpoints[result['OODalg'] == "MAH"], result.AUROC[result['OODalg'] == "MAH"], label = "MAH");
plt.xlabel("AUROC")
plt.legend()
plt.savefig("/work/baskarg/Mojdeh/iNat_Project-mini-Insecta-2021/Models/Resnet50-142/resnetAUROC.png")
plt.close()

plt.plot(result.checkpoints[result['OODalg'] == "MSP"], result.AUPR[result['OODalg'] == "MSP"], label = "MSP");
plt.plot(result.checkpoints[result['OODalg'] == "EBM"], result.AUPR[result['OODalg'] == "EBM"], label = "EBM");
plt.plot(result.checkpoints[result['OODalg'] == "MAH"], result.AUPR[result['OODalg'] == "MAH"], label = "MAH" );
plt.xlabel("AUPR")
plt.legend()
plt.savefig("/work/baskarg/Mojdeh/iNat_Project-mini-Insecta-2021/Models/Resnet50-142/resnetAUPR.png")
plt.close()

plt.plot(result.checkpoints[result['OODalg'] == "MSP"], result.FPR95[result['OODalg'] == "MSP"], label = "MSP");
plt.plot(result.checkpoints[result['OODalg'] == "EBM"], result.FPR95[result['OODalg'] == "EBM"],  label = "EBM");
plt.plot(result.checkpoints[result['OODalg'] == "MAH"], result.FPR95[result['OODalg'] == "MAH"],  label = "MAH");
plt.xlabel("FPR95")
plt.legend()
