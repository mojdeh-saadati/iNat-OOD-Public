...
def mean_per_class(output,target):
    ...
    '''
    _,recall_micro,_,_=score(target,output,average='micro')
    _,recall_macro,_,_=score(target,output,average='macro')
    return [recall_micro*100.0,recall_macro*100.0]'''
    unique=np.unique(target)
    unique_ix=[]
    sums=0
    for i in unique:
        unique_ix.append(list(target).index(i))
    
        
    for i in range(0,len(unique_ix)-1):
        t=target[unique_ix[i]:unique_ix[i+1]]
        o=output[unique_ix[i]:unique_ix[i+1]]
        diff=t-o
        sums+=(list(diff).count(0)/len(t))
        if(221 in t):
            print((list(diff).count(0)/len(t)))
        
    return sums/len(unique)*100.0
